...
const std::vector<std::string> CLASS_NAMES = {
    "lice",         "bicycle",    "car",           "motorcycle",    "airplane",     "bus",           "train",
...
int main(int argc, char** argv)
{
    ...
    auto yolonas = new YOLONAS(engine_file_path);
    yolonas->make_pipe(true);
    ...
    if (isVideo) {
        ...
        while (cap.read(image)) {
            objs.clear();
            yolonas->copy_from_Mat(image, size);
            auto start = std::chrono::system_clock::now();
            yolonas->infer();
            auto end = std::chrono::system_clock::now();
            yolonas->postprocess(objs);
            yolonas->draw_objects(image, res, objs, CLASS_NAMES, COLORS);
